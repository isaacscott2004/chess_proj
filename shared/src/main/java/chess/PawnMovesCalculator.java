...
public class PawnMovesCalculator implements PieceMovesCalculator {
    ...
    private void initialPosition(ArrayList<ChessMove> potentialMoves, int rowDirection, ChessPosition myPosition, ChessBoard board) {
        if (!(board.isPieceOnSquare(myPosition.getRow() + rowDirection, myPosition.getColumn()))) {
            potentialMoves.add(new ChessMove(myPosition, new ChessPosition(myPosition.getRow() + rowDirection, myPosition.getColumn()), null));
            if (!(board.isPieceOnSquare(myPosition.getRow() + (rowDirection * 2), myPosition.getColumn()))) {
                potentialMoves.add(new ChessMove(myPosition, new ChessPosition(myPosition.getRow() + (rowDirection * 2), myPosition.getColumn()), null));
                }
            }
    }

    private void middlePosition(ArrayList<ChessMove> potentialMoves, int rowDirection, ChessPosition myPosition, ChessBoard board){
        if(!(board.isPieceOnSquare(myPosition.getRow() + rowDirection, myPosition.getColumn()))) {
            potentialMoves.add(new ChessMove(myPosition, new ChessPosition(myPosition.getRow() + rowDirection, myPosition.getColumn()), null));
        }
    }
